...
int main(int argc, char **argv){
    ...
    ptrdiff_t local_1_start = fft_h.local_n1_offset;
    ptrdiff_t local_allocation = fft_h.local_size_grid;

    diffusivity = (double*)malloc(local_allocation*sizeof(double));
    conc = (double*)malloc(local_allocation*sizeof(double));
    dconc = (double*)malloc(local_allocation*sizeof(double));
    aux1 = (double*)malloc(local_allocation*sizeof(double));
    aux2 = (double*)malloc(local_allocation*sizeof(double));

    //printf("\n%d %d %d\n", local_n1, local_1_start, local_allocation);
	...
	f3conc = exp( -pow((x3-0.5*L3)/rad_conc,2));
        ...
        for (i2 = 0; i2 < n2; ++i2)
	  {
	    ...
	    for (i1 = local_1_start; i1 < local_1_start + local_n1; ++i1)
	      {
		x1=L1*((double)i1)/n1;
		f1diff = exp( -pow((x1-0.5*L1)/rad_diff,2));
		f1conc = exp( -pow((x1-0.5*L1)/rad_conc,2));

		index = index_f(i1 - (int)local_1_start, i2, i3, (int)local_n1, n2, n3);
     ...
     *
     */
    plot_data_2d("diffusivity", n1, n2, n3, local_n1, local_1_start, 1, diffusivity);
    plot_data_2d("diffusivity", n1, n2, n3, local_n1, local_1_start, 2, diffusivity);
    plot_data_2d("diffusivity", n1, n2, n3, local_n1, local_1_start, 3, diffusivity);
    ...
    double ss_tmp = ss;
    MPI_Allreduce(&ss_tmp, &ss, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

    ss = 1.0/(ss*fac);
    for (i1=0; i1< local_n1*n2*n3; ++i1)
      conc[i1]*=ss;
    ...
    for (istep = 1; istep <= nstep; ++istep)
        ...
        if (istep%30 == 1)
	  {
                ...
                for (i2 = 0; i2 < n2; ++i2)
		  {
			...
			index = index_f(i1-local_1_start, i2, i3, local_n1, n2, n3);
			ss += conc[index];
	     ...
	     */
            ...
            double r2mean_tmp = r2mean;
            MPI_Allreduce(&r2mean_tmp, &r2mean, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

            ss *= fac;
            r2mean *= fac;
            end = seconds();

            double  slowest_comp, comp_time = (end-start)/istep;
            MPI_Reduce(&comp_time, &slowest_comp, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);

            if (rank == 0){
#if 1
              printf( " %d %17.15f %17.15f Elapsed time per iteration %f \n ",
                      istep, r2mean, ss, slowest_comp);
#else
              printf("%f\t", slowest_comp);
#endif
            }

            // HINT: Use parallel version of output routines
            plot_data_2d("concentration", n1, n2, n3, local_n1, local_1_start, 2, conc);
            plot_data_1d("1d_conc", n1, n2, n3, local_n1, local_1_start, 3, conc);
	  }
