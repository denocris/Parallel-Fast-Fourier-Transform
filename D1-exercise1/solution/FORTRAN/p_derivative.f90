...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  real(8), dimension(n1, n2, local_n3), intent(inout) :: data
  real(8), dimension(n1, n2, local_n3), intent(out) :: deriv
  ...
  ! 
     ...
     allocate(aux(n1, n2, local_n3),stat=status)
     ...
     if(ipol.eq.1) then
        ...
        do i1=1,n2
           ...
           if(i1-1.eq.n2/2) i = 0 !i-n2
           aux(:,i1,:) = aux(:,i1,:) * cmplx(0.d0,G*i,8)
        enddo
     else if(ipol.eq.3) then
        G = 2.d0*pi/L3
        do i1=1,local_n3
           i = i1-1+local_n3_offset
           if(i.gt.n3/2) i = i-n3
           if(i.eq.n3/2) i = 0 !i-n3
           aux(:,:,i1) = aux(:,:,i1) * cmplx(0.d0,G*i,8)
        enddo
     else
